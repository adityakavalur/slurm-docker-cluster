...
class CreateConf(object):
  ...
  def __readFromFile(self):
    with open('/data/db_credentials.txt') as file:
      if (not self.__host):   self.__host   = file.readline().strip('\n\r')
      if (not self.__user):   self.__user   = file.readline().strip('\n\r')
      if (not self.__passwd): self.__passwd = file.readline().strip('\n\r')
      if (not self.__db):     self.__db     = file.readline().strip('\n\r')
